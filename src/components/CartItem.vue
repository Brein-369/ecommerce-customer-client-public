  ...
  <tr>
    <td>
      <div class="img-container">
        <img
          :src="eachCart.Product.image_url"
          alt="..."
        />
      </div>
    </td>
    <td class="td-name">
      <p>{{ eachCart.Product.name }}</p>
    </td>
    <td class="td-number text-center">{{filteredPrice}}</td>
    <td class="td-number text-center">
      <div class="btn-group">
        <button class="btn btn-round btn-info btn-sm" @click.prevent="subQuantity(eachCart.id)">
          <i class="fa fa-1x fa-minus-circle" aria-hidden="true"></i>
        </button>
        <h4 class="my-0 mx-3">{{eachCart.quantity}}</h4>
        <button class="btn btn-round btn-info btn-sm" @click.prevent="addQuantity(eachCart.id)">
    ...
    <td class="td-number text-center">
      <div class="btn-group">
        <button class="btn btn-round btn-info btn-sm" @click.prevent="deleteCart(eachCart.id)">
...
export default {
  ...
  computed: {
    filteredPrice () {
      return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(this.eachCart.Product.price)
    }
  },
  methods: {
    deleteCart (cartId) {
    ...
    addQuantity (cartId) {
      console.log(cartId)
      if (this.eachCart.quantity < this.eachCart.Product.stock) {
        this.$store.dispatch('addQuantity', cartId)
      } else {
        console.log('sorry this product quanity has reach stock limit')
      }
    },
    subQuantity (cartId) {
      console.log(cartId)
      this.$store.dispatch('subQuantity', cartId)
    }
  }
}
</script>
